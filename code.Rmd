...
trimmed_squared_residuals_test <- (trimmed_residuals_test - trimmed_mean_test)^2
# Calculate the robust RMSE using the trimmed mean:
robust_rmse_step <- sqrt(mean(trimmed_squared_residuals_test))
...
results_table <- data.frame(
  ...
  Value = c((optimal_components - 1), r2_train, r2_adj_train, RMSE_train, RMSE_test)
)
...
robust_rmse_best <- sqrt(mean(trimmed_squared_residuals_best))
```
...
mr <- lm.ridge(total_UPDRS ~ Jitter_p + Jitter_Abs + Jitter_RAP + Jitter_PPQ5 + Jitter_DDP + Shimmer + Shimmer_dB + Shimmer_APQ3 + Shimmer_APQ5 + Shimmer_APQ11 + Shimmer_DDA + NHR + HNR + RPDE + DFA + PPE, data = train_data, lambda=(seq(0, 0.1, 0.001)))
...
mr <- lm.ridge(total_UPDRS ~ Jitter_p + Jitter_Abs + Jitter_RAP + Jitter_PPQ5 + Jitter_DDP + Shimmer + Shimmer_dB + Shimmer_APQ3 + Shimmer_APQ5 + Shimmer_APQ11 + Shimmer_DDA + NHR + HNR + RPDE + DFA + PPE, data = train_data, lambda=lGCV)
```

```{r}
predictions_test = cbind(1,as.matrix(data_test)) %*% coef(mr)
predictions_train = cbind(1,as.matrix(data_train)) %*% coef(mr)
...
# Calculate the robust RMSE using the trimmed mean:
robust_rmse_ridge <- sqrt(mean(trimmed_squared_residuals_test))
```
...
trimmed_squared_residuals_test <- (trimmed_residuals_test - trimmed_mean_test)^2
# Calculate the robust RMSE using the trimmed mean:
clean_robust_rmse_lineal <- sqrt(mean(trimmed_squared_residuals_test))
...
trimmed_squared_residuals_test <- (trimmed_residuals_test - trimmed_mean_test)^2
# Calculate the robust RMSE using the trimmed mean:
clean_robust_rmse_step <- sqrt(mean(trimmed_squared_residuals_test))
...
# Number of observations to trim
n_trim_best <- round(0.1 * length(residuals_best))
# Sort the residuals in ascending order
sorted_residuals_best <- sort(residuals_best)
# Trim the specified percentage of observations from both ends
trimmed_residuals_best <- sorted_residuals_best[(n_trim_best + 1):(length(residuals_best) - n_trim_best)]
# Calculate the trimmed mean of the residuals for the best model
trimmed_mean_best <- mean(trimmed_residuals_best)
# Calculate the squared residuals using the trimmed mean for the best model
trimmed_squared_residuals_best <- (trimmed_residuals_best - trimmed_mean_best)^2
# Calculate the robust RMSE using the trimmed mean for the best model
clean_robust_rmse_best <- sqrt(mean(trimmed_squared_residuals_best))
...
trimmed_squared_residuals_test <- (trimmed_residuals_test - trimmed_mean_test)^2
# Calculate the robust RMSE using the trimmed mean:
clean_robust_rmse_ridge <- sqrt(mean(trimmed_squared_residuals_test))
```
```{r}
# Create a data frame for the results
results <- data.frame(
...
results2 <- data.frame(
  ...
  RMSE_Con = c(rmse_test_lineal, rmse_test_AIC, best_rmse, rmse_test_ridge),
  RMSE_Sin = c(rmse_clean_model_lineal, rmse_clean_model_step, clean_best_rmse, cleanm_ridge_rmse),
  Robusto_Con = c(robust_rmse_lineal, robust_rmse_step, robust_rmse_best, robust_rmse_ridge),
  Robusto_Sin = c(clean_robust_rmse_lineal, clean_robust_rmse_step, clean_robust_rmse_best, clean_robust_rmse_ridge),
)
