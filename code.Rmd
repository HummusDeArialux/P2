...
Dadas las limitaciones identificadas en los modelos del ejercicio anterior, se ha procedido a eliminar los 3 puntos más influyentes, determinados mediante el cálculo de la distancia de Cook, del modelo lineal. Posteriormente, se ha vuelto a ejecutar el mismo código utilizado previamente, utilizando los nuevos datos resultantes de la eliminación de los puntos influyentes.

```{r}
# Código 18: Eliminación de puntos influyentes
...
# Create a data frame for the results
results_table <- data.frame(
  Metric = c("Número de variables", "R-squared", "Adjusted R-squared", "RMSE_train", "RMSE test"),
...
optimal_components = optimal_components - 1
```
```{r}
# Predict on the training data using the model
...
Como se observa en la tabla de resultados, el valor del RMSE apenas experimenta cambios significativos tras la eliminación de los tres puntos más influyentes. Solo en el modelo basado en el análisis de componentes principales se aprecia una ligera mejora en el RMSE, aunque la diferencia es tan mínima que no puede considerarse como una mejora sustancial en el ajuste del modelo.

En resumen, la falta de mejora o incluso el empeoramiento del RMSE después de eliminar los puntos influyentes puede deberse a diversas razones, como una identificación incorrecta de los puntos influyentes, la eliminación de información valiosa o un sesgo en las predicciones debido a la distribución de los puntos influyentes. Es fundamental tener en cuenta estos factores y evaluar el impacto de la eliminación de los puntos influyentes en el contexto específico de nuestros modelos y conjunto de datos.

### (b) Cálculo del RMSE robusto

A diferencia del cálculo tradicional del RMSE que utiliza todos los puntos de datos, el RMSE robusto con media recortada al 10% elimina una fracción de los valores atípicos más extremos antes de calcular el error. Al emplear esta técnica, el RMSE robusto con media recortada al 10% se vuelve menos sensible a los valores atípicos y proporciona una medida más robusta del error. Esta medida resulta útil cuando se sospecha la presencia de valores atípicos en el conjunto de datos, los cuales podrían tener un impacto desproporcionado en la estimación del error.
...
results2 <- data.frame(
  ...
  RMSE_Con = c(lineal_rmse_test, step_rmse_test, PCA_rmse_test, ridge_rmse_test),
  RMSE_Sin = c(lineal_clean_rmse_test, step_clean_rmse_test, PCA_clean_rmse_test, ridge_clean_rmse_test),
  RMSE_Robusto_Con = c(robust_lineal_rmse_test, robust_step_rmse_test, robust_PCA_rmse_test, robust_ridge_rmse_test),
  RMSE_Robusto_Sin = c(robust_lineal_clean_rmse_test, robust_step_clean_rmse_test, robust_PCA_clean_rmse_test, robust_ridge_clean_rmse_test)
)

print(results2)
```

Como se puede apreciar, en todos los casos se observa una reducción aproximada de 3 unidades del RMSE al calcularlo con la media recortada. La diferencia en el RMSE entre los dos enfoques de cálculo puede atribuirse a la influencia de los valores atípicos en el cálculo del error. Al utilizar la media recortada, se eliminan los valores extremos, lo que puede disminuir la influencia de dichos puntos atípicos en la estimación del error cuadrático medio.

Dado que no se observan diferencias significativas en el RMSE robusto al considerar o no los valores atípicos, sería recomendable realizar una revisión y limpieza más exhaustiva de los datos, ya que la eliminación de tan solo tres puntos no genera cambios significativos.

Otra opción a considerar sería la transformación de variables, ya que ciertas transformaciones podrían ayudar a reducir la influencia de los valores atípicos. También se podría contemplar el uso de métodos robustos de regresión, los cuales son menos sensibles a los valores atípicos.

### (c) LTS o Huber

La regresión Huber es un método robusto utilizado para ajustar modelos de regresión que son menos sensibles a los valores atípicos o a los errores no normales presentes en los datos (Código 18). Este enfoque utiliza una función de pérdida modificada para estimar los coeficientes de regresión. Dicha función combina la suma de errores cuadráticos para valores pequeños de error, y una pérdida lineal para valores grandes de error, lo que le permite ser menos sensible a los valores atípicos.

```{r}
# Código 18: Ajuste de regresión Huber, ejercicio 3c
set.seed(123)
...
Al aplicar la regresión Huber en nuestro estudio, no se observa una mejora significativa en las métricas calculadas anteriormente. El valor del RMSE se mantiene alrededor de las 10 unidades, lo cual sugiere que, a pesar de reducir la influencia de los datos atípicos, hay otros factores que impiden que nuestro modelo sea un buen predictor.

Además, el bajo valor de R2 indica que puede haber un problema relacionado con las variables predictoras utilizadas, ya que solo son capaces de explicar aproximadamente el 10% de la variabilidad en la puntuación total de UPDRS. Esto sugiere que podrían existir otras variables o factores no considerados en nuestro modelo que desempeñan un papel importante en la predicción de la puntuación total de UPDRS.

En conclusión, el análisis realizado hasta el momento revela varias limitaciones y desafíos en la construcción de un modelo predictivo para la puntuación total de UPDRS utilizando las medidas de voz como variables predictoras. A pesar de los esfuerzos realizados, los modelos de regresión lineal y otros enfoques explorados han mostrado un mal ajuste y una baja capacidad predictiva.
